...
namespace SmsAero
{
    public class SmsAeroClient
    {
        ...
        private readonly bool _debug = true; // Вывод информации об отправке
        ...
        public SmsAeroClient(string smsAeroLogin, string smsAeroApiKey, string from = "SMS Aero")
        {
            _from = UrlEncode(from);

            var basicAuthData = Convert.ToBase64String(Encoding.GetEncoding("ISO-8859-1").GetBytes(smsAeroLogin + ":" + smsAeroApiKey));
            _http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", basicAuthData);
            _http.DefaultRequestHeaders.UserAgent.ParseAdd("SACSharpClient/3.0.0");
        ...
        /// <returns>string(json)</returns>
        public async Task<string> ContactAdd(string number, int? groupId = null, DateTime? birthday = null,
            Sex? sex = null,
        ...
        /// <param name="sname">Отчество абонента</param>
        /// <returns>string(json)</returns>
        public async Task<string> ContactList(string number = "", int? groupId = null, DateTime? birthday = null,
            Sex? sex = null,
        ...
        private async Task<string> Send(string data, string method, HttpMethod httpMethod = null)
        {
            ...
            if (_debug) _PrintDebug(url, responseBody);
